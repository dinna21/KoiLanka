...
export default function Navbar() {
  ...
  return (
      <nav
        className="fixed top-0 z-50 left-1/2 transform -translate-x-1/2 w-[95%] sm:w-[90%] max-w-5xl px-4 sm:px-6 lg:px-8 border-b shadow-sm rounded-xl mt-4 bg-white/10 backdrop-blur-md border-white/30 text-white"
      >
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`relative text-sm md:text-base px-2 py-1 hover:text-orange-400 transition-colors duration-300 ${
                location.pathname === item.path ? 'font-semibold text-orange-300' : 'text-white'
              }`}
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`block px-4 py-2 rounded-lg hover:bg-orange-500 transition-colors duration-300 ${
                location.pathname === item.path ? 'bg-orange-600 font-semibold' : ''
              }`}
              onClick={handleLinkClick}
            >
              {item.label}
